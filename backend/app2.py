...
def predict():
    ...
    pred_mob = mobilenet.predict(input_tensor)[0]
    pred_res = resnet.predict(input_tensor)[0]
    pred_eff = efficientnet.predict(input_tensor)[0]

    ensemble_pred = (0.5 * pred_mob + 0.25 * pred_res + 0.25 * pred_eff)
    top_indices = ensemble_pred.argsort()[-3:][::-1]
    results = [{'label': class_names[i], 'confidence': float(ensemble_pred[i])} for i in top_indices]
